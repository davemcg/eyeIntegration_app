               ...
               navbarMenu('Data',
                          ...
                          tabPanel('Data Download',
                                   fluidPage(
                                     ...
                                     fluidRow(tags$a(href='https://hpc.nih.gov/~mcgaugheyd/eyeIntegration/2017_metadata_01.tsv.gz',
                                                     '2017_metadata_01.tsv.gz')),
                                     fluidRow(tags$a(href='https://hpc.nih.gov/~mcgaugheyd/eyeIntegration/2017_gene_TPM_01.tsv.gz',
                                                     '2017_gene_TPM_01.tsv.gz')),
                                     fluidRow(tags$a(href='https://hpc.nih.gov/~mcgaugheyd/eyeIntegration/2017_tx_TPM_01.tsv.gz',
                                                     '2017_tx_TPM_01.tsv.gz')),
                                     fluidRow(tags$a(href='https://hpc.nih.gov/~mcgaugheyd/eyeIntegration/2019_metadata_01.tsv.gz',
                                                     '2019_metadata_01.tsv.gz')),
                                     fluidRow(tags$a(href='https://hpc.nih.gov/~mcgaugheyd/eyeIntegration/2019_gene_TPM_01.tsv.gz',
                                                     '2019_gene_TPM_01.tsv.gz')),
                                     fluidRow(tags$a(href='https://hpc.nih.gov/~mcgaugheyd/eyeIntegration/2019_tx_TPM_01.tsv.gz',
                                                     '2019_tx_TPM_01.tsv.gz')),
                          ...
                          tabPanel('FAQs', fluidPage(
                            ...
                            navlistPanel("Pan-Tissue Expression FAQs", 
                                         tabPanel("How do I use the Pan-Tissue Expression section?",
                                                  "First you pick the dataset (2017 or 2019) [1]. Then you can tweak the 'Genes' [2] and 'Tissues' [3] by clicking in them and starting to type (allowed values will auto-fill). You can also delete values by clicking on them and hitting the 'delete' key on your keyboard. You can tweak the display of the box plots a bit by changing the 'Number of columns' field [4]. A higher number will squeeze more plots in each row. When you are done tweaking those parameters, click the big blue '(Re)Draw Plot!' button and wait a couple of seconds.", br(), br(), 'If you mouse over a data point, you will get metadata about that particular sample.', br(), br(), img(src='pantissue_screenshot.jpg', width = 900), br(), br()),
                                         tabPanel("What Pan-Tissue Expression data is displayed?",
                                                  "Each gene gets its own box. The y-axis is length scaled TPM (log2 transformed). The x axis is samples, colored by tissue. The right panels [6, 7] are tables with external links to gene info [6] and the absolute TPM values and the rank of the gene in the particular tissue (lower is more highly expressed) [7].", br(), br(), img(src='pantissue_screenshot.jpg', width = 900), br(), br()),
                                         tabPanel("How do I interpret the 'Decile' column in the data table?",
                                                  "To give a rough sense of how highly expressed the gene is in the tissue, the decile of expression is given in [7]; 10 is the highest decile of expression and 1 is the lowest.", br(), br(), img(src='pantissue_screenshot.jpg', width = 900), br(), br()),
                                         ...
                                         tabPanel("What is the 'Heatmap' radio button?",
                                                  "This produces a 2D visualization, with each gene as a row and each tissue as a column. More yellow is more expressed. It is a efficient way to display the expression of many genes and tissues.", br(), br(), img(src='pantissue_heatmap.jpg', width = 900), br(), br()),
                                         tabPanel("Custom Gene / Tissue combination via the URL",
                                                  "If you use the Pan-Tissue Boxplot feature a lot, you may find it frustrating to have to input in your favorite genes and tissues. We have added the ability to use a custom url to load in the genes and tissues of your choice. Previously you had to build this link youself - but now there's a handy button [5] you can click that will re-create the parameters. One downside is that the web app is continually using the URL dataset, which makes it impossible for you to change it. You can simply reload the web page with the custom bits.", br(), br(), img(src='pantissue_screenshot.jpg', width = 900), br(), br()),
                            ...
                            navlistPanel("2D Clustering FAQ",
                                         ...
                                         tabPanel("What is Mouse Retina Single Cell...?",
                                                  "Each data point is a single cell from the ", tags$a(href="https://www.biorxiv.org/content/early/2018/07/30/378950","Macosko and McCarroll"), 'or the ', tags$a(href="https://www.ncbi.nlm.nih.gov/pubmed/26000488","Clark and Blackshaw"), 'Dimensionality reduction with done with the t-SNE (Macosko) or UMAP (Clark) algorithm. Cluster assignments were taken from the respective papers. While we did the t-SNE on the Macosko data, the Clark authors provided the UMAP coordinates. The Clark dataset was generated across multiple time-points during development and thus, you can select time points of interest.')),
