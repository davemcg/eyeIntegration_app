...
# Intro
This is a quick guide on how to use the counts data from EiaD with your own, personal, RNA-seq data. 
...
## Back to R with tximport loading
I've run the above command and provide the `quant.sf` via a web link. 
```{r}
# import with tximport if you ran the above chunk
# txi <- tximport(c('ERR2303761_quant/quant.sf'), type = 'salmon', countsFromAbundance = 'no', txOut = FALSE, tx2gene = anno[,c(3,2)])

# running with the precomputed file
system('wget https://hpc.nih.gov/~mcgaugheyd/eyeIntegration/ERR2303761.quant.sf')
txi <- tximport(c('ERR2303761.quant.sf'), type = 'salmon', countsFromAbundance = 'no', txOut = FALSE, tx2gene = anno[,c(3,2)])
```

# Glue EiaD data into the `txi` object

We'll take 5 random adult cornea samples and 5 random adult retina samples
```{r}
EiaD_samples <- metadata %>% 
  filter(Sub_Tissue %in% c('Cornea - Adult Tissue', 'Retina - Fetal Tissue'), Kept == 'Kept') %>% 
  group_by(Sub_Tissue) %>% 
  sample_n(5) %>% 
  pull(sample_accession) %>% 
  unique()

EiaD_tissues <- metadata %>% select(sample_accession, Sub_Tissue) %>% unique() %>% filter(sample_accession %in% EiaD_samples) %>% pull(Sub_Tissue)
...
```{r}
library(DESeq2)
sampleTable <- data.frame(condition = factor(c("ERR2303761", EiaD_tissues)))
rownames(sampleTable) <- colnames(txi$counts)
dds <- DESeqDataSetFromTximport(txi, sampleTable, ~condition)
```

# You can also use edgeR, or limma, etc. 
