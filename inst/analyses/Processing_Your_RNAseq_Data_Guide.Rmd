...
metadata <- read_csv("https://hpc.nih.gov/~mcgaugheyd/eyeIntegration/2023/eyeIntegration22_meta_2023_03_03.csv.gz")
gene_counts <- read_csv("https://hpc.nih.gov/~mcgaugheyd/eyeIntegration/2023/gene_counts.csv.gz")
```
...
```{r}
# This step may take a while as the file we are downloading from the web is about 44 MB in size
...
```{r}
# match up row names
EiaD_data <- data.frame(gene_counts[, c(EiaD_retina_samples)])
row.names(EiaD_data) <- str_extract(gene_counts$Gene, "(?<=\\().+?(?=\\))")
# rows to keep
EiaD_keep <- row.names(EiaD_data) %in% row.names(txi$counts)
EiaD_data_subset <- EiaD_data[EiaD_keep, c(EiaD_retina_samples)]
...
top100 %>%
  ...
  left_join(conversion_table, by = join_by(ensembl_gene_id)) %>% 
